...
//        $grid->column('body', __('Body'))->style("width:50px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;");
        $grid->user()->name('用户名');
        $grid->category()->name('分类')->width('50');

//        $grid->column('user_id','用户名')->display(function (){
//            dd();
////            return User::find($userId)->name;
//        });
//        $grid->column('category_id', __('Category id'));
        $grid->column('reply_count','回复数量', __('Reply count'));
        $grid->column('view_count','浏览数量', __('View count'));
